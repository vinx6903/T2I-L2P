...
class ZImageTrainingModule(DiffusionTrainingModule):
    def __init__(
        ...
        # ============================================================
        # Optional VRAM optimization: keep text_encoder on CPU during training,
        # ============================================================
        self.offload_text_encoder = offload_text_encoder
        # Expose flag on pipe so the prompt-embedder unit can pick it up.
        self.pipe.offload_text_encoder = offload_text_encoder
        if (
            offload_text_encoder
            and getattr(self.pipe, "text_encoder", None) is not None
            and not getattr(self.pipe, "vram_management_enabled", False)
        ):
...
def z_image_parser():
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Keep text_encoder on CPU during training and only move it to GPU "
            "Recommended for 24GB cards. See train_run_low_VRAM.sh."
        ),
    )
    return parser
