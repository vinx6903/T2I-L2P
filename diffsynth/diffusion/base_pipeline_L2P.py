...
class BasePipeline(torch.nn.Module):
    ...
    def freeze_except(self, model_names):
        ...
        for name in model_names:
            ...
            print(f"Configuring partial fine-tuning for model: {name}")
            module.train()
            module.requires_grad_(True)

            if hasattr(module, 'layers'):
                num_layers = len(module.layers)
                indices_to_freeze = list(set(range(5, num_layers-5)))
                
                for i in indices_to_freeze:
                    if i < num_layers: # 安全检查
                        module.layers[i].eval()
                        module.layers[i].requires_grad_(False)
            


        # ------------------------------------------------------
        # Count and output the number of trainable parameters.
        # ------------------------------------------------------
        trainable_params = 0
        all_params = 0
        ...
        print("=" * 50)
        print(f"Trainable Parameters Info:")
        print(f"  - Total Parameters: {all_params:,}")
        print(f"  - Trainable Parameters: {trainable_params:,}")
        if all_params > 0:
            print(f"  - Trainable Ratio: {trainable_params/all_params:.2%}")
        print("=" * 50)
