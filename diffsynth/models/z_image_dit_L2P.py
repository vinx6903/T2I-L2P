...
class MicroDiffusionModel(nn.Module):
    ...
    def forward(self, x, c, use_gradient_checkpointing=False, use_gradient_checkpointing_offload=False):
        ...
        # ---------- stage 1 (encoder): H -> H/2 ----------
        def _enc_stage_1(x_in):
            enc1_out = self.enc1(x_in)              # [B, 64, H,   W]
            p1_out   = self.pool1(enc1_out)         # [B, 64, H/2, W/2]
            return enc1_out, p1_out

        # ---------- stage 2 (encoder): H/2 -> H/4 ----------
        def _enc_stage_2(p_in):
            enc2_out = self.enc2(p_in)              # [B, 128, H/2, W/2]
            p2_out   = self.pool2(enc2_out)         # [B, 128, H/4, W/4]
            return enc2_out, p2_out

        # ---------- stage 3 (encoder): H/4 -> H/8 ----------
        def _enc_stage_3(p_in):
            enc3_out = self.enc3(p_in)              # [B, 256, H/4, W/4]
            p3_out   = self.pool3(enc3_out)         # [B, 256, H/8, W/8]
            return enc3_out, p3_out

        # ---------- stage 4 (encoder + bottleneck): H/8 -> H/16 -> bottleneck ----------
        def _enc_stage_4_and_bottleneck(p_in, c_in):
            ...
            bottleneck_input = torch.cat([p4_out, c_in], dim=1)
            bottleneck_out = self.bottleneck(bottleneck_input)  # [B, 512, H/16, W/16]
            return enc4_out, bottleneck_out

        # ---------- stage 5 (decoder): up4 + dec4 ----------
        def _dec_stage_4(bottleneck_in, enc4_in):
            dec4_out = self.up4(bottleneck_in)              # [B, 512, H/8, W/8]
            dec4_out = torch.cat([dec4_out, enc4_in], dim=1)
            dec4_out = self.dec4(dec4_out)                  # [B, 256, H/8, W/8]
            return dec4_out

        # ---------- stage 6 (decoder): up3 + dec3 ----------
        def _dec_stage_3(dec_in, enc3_in):
            dec3_out = self.up3(dec_in)                     # [B, 256, H/4, W/4]
            dec3_out = torch.cat([dec3_out, enc3_in], dim=1)
            dec3_out = self.dec3(dec3_out)                  # [B, 128, H/4, W/4]
            return dec3_out

        # ---------- stage 7 (decoder): up2 + dec2 ----------
        def _dec_stage_2(dec_in, enc2_in):
            dec2_out = self.up2(dec_in)                     # [B, 128, H/2, W/2]
            dec2_out = torch.cat([dec2_out, enc2_in], dim=1)
            dec2_out = self.dec2(dec2_out)                  # [B, 64,  H/2, W/2]
            return dec2_out

        # ---------- stage 8 (decoder): up1 + dec1 + out_conv ----------
        def _dec_stage_1_and_out(dec_in, enc1_in):
            dec1_out = self.up1(dec_in)                     # [B, 64, H, W]
            dec1_out = torch.cat([dec1_out, enc1_in], dim=1)
            dec1_out = self.dec1(dec1_out)                  # [B, 64, H, W]
            x_out = self.out_conv(dec1_out)                 # [B, C,  H, W]
            return x_out
        ...
        # Encoder
        enc1_out, p1_out          = _gc(_enc_stage_1, x)
        enc2_out, p2_out          = _gc(_enc_stage_2, p1_out)
        enc3_out, p3_out          = _gc(_enc_stage_3, p2_out)
        enc4_out, bottleneck_out  = _gc(_enc_stage_4_and_bottleneck, p3_out, c)

        # Decoder
        dec4_out = _gc(_dec_stage_4, bottleneck_out, enc4_out)
        dec3_out = _gc(_dec_stage_3, dec4_out,       enc3_out)
        dec2_out = _gc(_dec_stage_2, dec3_out,       enc2_out)
        x_out    = _gc(_dec_stage_1_and_out, dec2_out, enc1_out)
